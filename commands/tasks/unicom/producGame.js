...
var producGame = {
  ...
  playGame: async (axios, options) => {
    ...
    let playGame = require(path.resolve(
      path.join(__dirname, "./playGame.json")
    ));
  ...
  doGameFlowTask: async (axios, options) => {
    let { popularList: allgames, jar } = await producGame.popularGames(
      axios,
      options
    );
  ...
  doGameIntegralTask: async (axios, options) => {
    ...
    games = games.filter(
      (d) =>
        d.task === "5" && d.reachState === "0" && d.task_type === "duration"
    );
    console.log("剩余未完成game", games.length);
    let queue = new PQueue({ concurrency: 30 });
    ...
    games = cgames.filter(
      (d) =>
        d.task === "5" && d.reachState === "1" && d.task_type === "duration"
    );
    console.log("剩余未领取game", games.length);
    for (let game of games) {
      await new Promise((resolve, reject) =>
